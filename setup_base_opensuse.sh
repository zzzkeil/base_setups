#!/bin/bash
echo " not ready, just a try to make this install possible on opensuse leap 16 "
exit
...
Base serverconfig for OpenSUSE Leap 16 only\n
...
if [[ "$ID" = 'opensuse-leap' ]]; then
    if [[ "$VERSION_ID" = '16' ]]; then
        systemos='opensuse-leap'
    fi
fi

if [[ "$systemos" = '' ]]; then
whiptail --title "Aborted" --msgbox "This script is only for OpenSUSE Leap 16 !" 15 80
exit 1
fi
...
update_upgrade_with_gauge() {
    {
        echo 10
        echo "Starting zypper refresh..."
        sudo zypper ref &> /dev/null
        if [ $? -ne 0 ]; then
            echo 100
            echo "Error: zypper refresh failed."
            exit 1
        fi
        ...
        if [ $? -ne 0 ]; then
            echo 100
            echo "Error: zypper update failed."
            exit 1
        fi
...
packages1=("firewalld" "fail2ban" "rsyslog")
